...
class Restler
{
     ...
     * @var string 
     */
    protected $api_class_prefix = 'OAuth2_API_';
    ...
    public function __construct($production_mode = FALSE)
    {
        ...
        $this->cache_dir       = defined('UC_DATADIR') ? (UC_DATADIR . 'oauth2') : getcwd();
    ...
    {
        if (!class_exists($this->api_class_prefix . $class_name)) {
        ...
        if (!$this->cached) {
            ...
            $this->generateMap($this->api_class_prefix . $class_name, $base_path);
        }
    }
    ...
    */
    ...
    {
        $path = urldecode($this->removeCommonPath(trim($_SERVER['REQUEST_URI'], '/'),
